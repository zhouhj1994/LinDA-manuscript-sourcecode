run.fun <- function(m, n, gamma, mu,  
                    ...
                    theta0 = NULL, kappa0 = NULL,
                    S1.zero.prop = NULL, 
                    model = c('S7.1', 'S7.2'), 
                    case = 'C0', alpha, n.met) {
  data <- simulate.data(m, n, gamma, mu, beta0, sigma2, eta0, theta0, kappa0,
                        S1.zero.prop, model, case)
  Y <- data$Y
  Z <- data$Z
  H <- data$H
  
  err <- try(rej.maaslin2 <- maaslin2.fun(Y, Z, 'u', 'id', alpha), 
             silent = TRUE)
  if(class(err) == 'try-error') rej.maaslin2 <- NA
  
  rej.list <- list(rej.maaslin2)
...
## Simulation setups
###############################################################
source('~/Documents/diff_abundance_analysis/LinDA-manuscript/LinDA-result/simulation/code/competing_methods.R')
source("~/Documents/diff_abundance_analysis/LinDA-manuscript/LinDA-result/simulation/code/simulate_data.R")
para0 <- readRDS("~/Documents/diff_abundance_analysis/LinDA-manuscript/LinDA-result/simulation/code/log.normal.para.rds")
...
output.raw <- foreach (i = 1 : nsim,.combine = 'rbind') %dopar% {
  ...
  sink(paste0(model, 'C0Maaslin2','_progress.txt'), append = TRUE) 
  print(i)
  sink()
  result
}
write.table(output.raw, paste0("output_raw_", model, "C0Maaslin2.txt"))
...
write.table(output, paste0("output_", model, "C0Maaslin2.txt"))
