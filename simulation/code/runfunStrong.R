run.fun <- function(m, n, gamma, mu,  
  ...
  data <- simulate.data(m, n, gamma, mu, beta0, sigma2, eta0, S1.zero.prop, 
                        model, case, strong = TRUE)
  Y <- data$Y
  Z <- data$Z
  H <- data$H
...
output.raw <- foreach (i = 1 : nsim,.combine = 'rbind') %dopar% {
  ...
  write.table(result, paste0("output_raw_", model, case, "Strong_", i, ".txt"))
  result
}

write.table(output.raw, paste0("output_raw_", model, case, "Strong.txt"))
...
write.table(output, paste0("output_", model, case, "Strong.txt"))
