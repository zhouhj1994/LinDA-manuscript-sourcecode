run.fun <- function(m, n, gamma, mu,  
  ...
  rej.list <- list(rej.linda, rej.maaslin2)
...
output.raw <- foreach (i = 1 : nsim,.combine = 'rbind') %dopar% {
  ...
  if(strong) {
    sink(paste0(model, case,'Strong_progress.txt'), append = TRUE)
  } else {
    sink(paste0(model, case,'_progress.txt'), append = TRUE) 
  }
  print(i)
  sink()
  result
...
if(strong) {
  write.table(output, paste0("output_", model, case, "Strong_Maaslin2LinDA", ".txt"))
} else {
  write.table(output, paste0("output_", model, case, "_Maaslin2LinDA", ".txt"))
}
