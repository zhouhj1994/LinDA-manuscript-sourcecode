run.fun <- function(m, n, gamma, mu,  
  ...
  res <- linda(Y, Z, formula, corr.cut = 0, alpha = alpha)
  rej2 <- which(res$output[[1]]$reject)
  
  res <- linda(Y, Z, formula, corr.cut = 1, alpha = alpha)
...
output.raw <- foreach (i = 1 : nsim,.combine = 'rbind') %dopar% {
  ...
  write.table(result, paste0("output_raw_", model, case, "LinDA_", i, ".txt"))
  result
}

write.table(output.raw, paste0("output_raw_", model, case, "LinDA.txt"))
...
write.table(output, paste0("output_", model, case, "LinDA.txt"))
